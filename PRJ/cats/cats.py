...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def func(s):
        s_ ="".join(lower(c) for c in s if isalpha(c) or c==" ")
        s_ = s_.split(" ")
        #print(s_)
        for word in subject:
            if word in s_:
...
def accuracy(typed, source):
    ...
    return cor*100 / len(typed_words)
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        for word in word_list:
            if diff_function(typed_word, word, limit) < cnt:
                cnt = diff_function(typed_word, word, limit)
                ans = word
...
big_limit = 2
print(minimum_mewtations("ckiteusabcdefghijklm", "kittensnopqrstuvwxyz", big_limit))
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[tpp[j][i] - tpp[j][i-1] for i in range(1, len(tpp[0]))] for j in range(len(tpp))]  # You may remove this line
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            (index, minimum) = (index, minimum) if times[j][i] >= minimum else (j, times[j][i])
