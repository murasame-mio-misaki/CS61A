...
def balanced(m):
    ...
    if is_planet(m):
        ...
        left_arm = left(m)
        right_arm = right(m)
        torque = lambda arm_ : total_mass(end(arm_)) * length(arm_)
...
def max_path_sum(t):
    ...
    def find_path(T , path):
        if is_leaf(T):
            ...
            return max(find_path(branch, path+label(T)) for branch in branches(T))
